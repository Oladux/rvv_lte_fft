...
# OpenOCD+Spike бывают медленными
set remotetimeout 120
set tcp connect-timeout 120

# Не использовать extended-remote
# Иначе OpenOCD remote_bitbang ломается на vMustReplyEmpty
target remote :3333
...
try:
    sym = gdb.parse_and_eval('&g_results')
except gdb.error:
...
try:
    ...
    raise gdb.GdbError("Failed to read g_results memory")

results = []

for i in range(N_SIZES):
    off = i * STRUCT_SIZE
    chunk = raw[off : off + STRUCT_SIZE]
    N, cyc_min, ins_min, cyc_med, ins_med, done = \
        struct.unpack_from('<6Q', chunk)
    results.append({
        'N':        N,
        'cyc_min':  cyc_min,
        'ins_min':  ins_min,
...
for r in results:
    ...
    print(
        '  | {:>4} | {:>10} | {:>10} | {:>10.3f} | {:>8.3f} | {:>8.3f} | {:>8.3f} | {:>8.5f} |'
        .format(
            int(N),
            cyc,
            ins,
